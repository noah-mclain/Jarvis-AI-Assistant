...
with open('kaggle_deepseek_unsloth.ipynb', 'r') as f:
    notebook = json.load(f)

# Clear existing cells after the setup cells (keep the first 5)
notebook['cells'] = notebook['cells'][:5]

# Update the title and introduction
...
# Update the package installation cell
...
# Add new cells for setting up paths properly
...
# Add imports matching run_finetune.py
notebook['cells'].append({
    "cell_type": "markdown",
    "metadata": {},
    "source": [
...
# Add the implementation of apply_memory_efficient_defaults from run_finetune.py
notebook['cells'].append({
    "cell_type": "markdown",
    "metadata": {},
    "source": [
...
# Add training function implementation
notebook['cells'].append({
    "cell_type": "markdown",
    "metadata": {},
    "source": [
...
# Add a cell to test the fine-tuned model
notebook['cells'].append({
    "cell_type": "markdown",
    "metadata": {},
    "source": [
...
# Add a cell to compress and save the model
notebook['cells'].append({
    "cell_type": "markdown",
    "metadata": {},
    "source": [
...
with open('kaggle_deepseek_unsloth_updated.ipynb', 'w') as f:
    json.dump(notebook, f, indent=2)

print("Updated Kaggle notebook created at 'kaggle_deepseek_unsloth_updated.ipynb'") 