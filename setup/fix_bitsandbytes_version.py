...
MIN_VERSION = "0.43.2"  # Minimum required version for 4-bit quantization with to() method
...
def install_compatible_version():
    ...
    try:
        logger.info(f"Trying to install bitsandbytes=={MIN_VERSION}")
        ...
        logger.warning(f"Failed to install bitsandbytes {MIN_VERSION}: {e}")
        ...
        try:
            ...
            try:
                ...
                logger.info(f"Installed bitsandbytes version: {version}")
            except ImportError:
                logger.warning("Could not import bitsandbytes after installation")

            return True
        except subprocess.CalledProcessError as e2:
            logger.error(f"Failed to install latest bitsandbytes: {e2}")

            # Try installing a known working version
            try:
                logger.info("Trying to install bitsandbytes 0.42.0")
                ...
                # Add a warning about 4-bit quantization
                logger.warning("Installed bitsandbytes 0.42.0, which may not fully support 4-bit quantization with to() method")
                logger.warning("You may need to use 8-bit quantization instead")

                return True
            except subprocess.CalledProcessError as e3:
                logger.error(f"Failed to install bitsandbytes 0.42.0: {e3}")
                return False
